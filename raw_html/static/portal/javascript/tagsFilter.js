...
interestTags=interestTags.map((item)=> {
        let value=item.id value=value.replace(/_/g, ' ');
        return value
    }

); //

for(let item of interestTags) {

    //appending the user interested tags
    $(".categories-container").append(`<button class="btn"onclick="filtercategory('${item}')">$ {
            item
        }
...
if (children<maxCategoriesLength) {
    ...
    for (let i=0; i<interestTags.length; i++) {
        ...
        if(index > -1) moreTags.splice(index, 1);
    }

    let numberOfAdditions=Math.min(maxCategoriesLength-children, moreTags.length) for(let i=0; i<numberOfAdditions; i++) {

        //adding more tags
        $(".categories-container").append(`<button class="btn"onclick="filtercategory('${moreTags[i]}')">$ {
                moreTags[i]
            }
